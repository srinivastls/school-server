...
const parseDateRange = (
  ...
  const fromDate =
    parseAttendanceDate(from);

  const toDate =
    parseAttendanceDate(to);
...
const getDailyStudentAttendance =
  async (
    req: any,
    res: any
  ) => {
    ...
    try {
      ...
      console.error(
        "GET DAILY STUDENT ATTENDANCE ERROR:",
        error
      );
  ...
  async (
    ...
    try {
      ...
      if (!dateRange) {
        return res.status(400).json({
          message:
            "Invalid date range. Use DD/MM/YYYY",
        });
      }
...
const getTeacherAttendanceReport =
  async (
    req: any,
    res: any
  ) => {
    ...
    try {
      ...
      console.error(
        "GET TEACHER ATTENDANCE REPORT ERROR:",
        error
      );
  ...
  async (
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          message:
            "classId, fromDate and toDate are required",
        });
      }
...
export const attendanceReportControllers = {
