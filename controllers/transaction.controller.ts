...
const recordTxn = async (
  ...
  try {
    ...
    if (
      ...
      !studentAdmissionNo ||
      !adminId ||
      (paymentMode === PaymentMode.wallet && !transactionId)
    ) {
      return res
        .status(400)
    ...
    if (
      +amountDetails.tie > +student.tiePendingAmount ||
    ...
    const { tie, diary, belt, arrears, tuitionFee, textBookFee, noteBookFee } =
      amountDetails;
    ...
    const txn = await prisma.$transaction(async (tx) => {
      ...
      await tx.student.update({
        ...
        data: {
          tiePendingAmount: `${
            +student.tiePendingAmount - +tie
          }`,

          beltPendingAmount: `${
            +student.beltPendingAmount - +belt
          }`,

          arrearsPendingAmount: `${
            +student.arrearsPendingAmount - +arrears
          }`,

          pendingTuitionFee: `${
            +student.pendingTuitionFee - +tuitionFee
          }`,

          pendingNotebookFee: `${
            +student.pendingNotebookFee - +noteBookFee
          }`,

          pendingTextbookFee: `${
            +student.pendingTextbookFee - +textBookFee
          }`,

          pendingDiaryAmount: `${
            +student.pendingDiaryAmount - +diary
          }`,
    ...
    return res.status(200).json({
      amount: txn.amount,
      paymentMode: txn.paymentMode as PaymentMode,
      date: txn.date,
...
const getStudentTxns = async (
  ...
  try {
    ...
    const response: StudentTxnsResponse = txns.map((txn) => ({
      date: txn.date,
      classNumber: txn.classNumber,
      id: txn.id,
      pendingAmount: txn.pendingAmount,
...
const getTotalTxnAmount = async (
  ...
  try {
    ...
    for (const txn of txns) {
      ...
      if (txn.paymentMode === PaymentMode.wallet) {
