...
exports.studentPromotionControllers = void 0;
...
const getPromotionStudents = async (req, res) => {
    try {
        const { sourceAcademicYearId, targetAcademicYearId, classId, } = req.query;
        /* ========================================================
           SCHOOL
        ======================================================== */
        ...
        /* ========================================================
           CLASS NUMBER HELPER
        ======================================================== */
        const getSuggestedClass = (currentClassNumber) => {
            /*
             ...
             */
            const number = Number(currentClassNumber);
            if (!Number.isInteger(number)) {
                return null;
            }
            const nextNumber = String(number + 1);
            return (targetClasses.find(item => item.classNumber ===
                nextNumber) ??
                null);
        };
        /* ========================================================
        ...
        const result = students.map(student => {
            ...
            return {
                ...
                suggestedClass: suggestedClass
                    ? {
                        id: suggestedClass.id,
                        classNumber: suggestedClass.classNumber,
                        displayName: suggestedClass.displayName,
                    }
                    : null,
                promotion: existingPromotion
                    ? {
                        ...
                        remark: existingPromotion.remark,
                        toClassId: existingPromotion.toClassId,
                        toSectionId: existingPromotion.toSectionId,
        ...
        const summary = {
            total: result.length,
            eligible: result.filter(student => student.eligible).length,
            notEligible: result.filter(student => !student.eligible).length,
            alreadyProcessed: result.filter(student => Boolean(student.promotion)).length,
        };
        /* ========================================================
           RESPONSE
        ======================================================== */
...
const processStudentPromotion = async (req, res) => {
    try {
        const { studentId, fromAcademicYearId, toAcademicYearId, toClassId, toSectionId, status, remark, } = req.body;
        /* ========================================================
           SCHOOL
        ======================================================== */
        const schoolId = getSchoolId(req);
        ...
        /* ========================================================
           AUTHENTICATED USER
        ======================================================== */
        const promotedByUserId = req.user?.id;
        if (!promotedByUserId) {
        ...
        if (!studentId ||
            ...
            !status) {
            return res.status(400).json({
                message: "studentId, fromAcademicYearId, toAcademicYearId and status are required",
            });
        }
        const validStatuses = [
            "PROMOTED",
        ...
        if (status !== "NOT_PROMOTED" &&
            (!toClassId ||
                !toSectionId)) {
            return res.status(400).json({
                message: "Target class and section are required",
            });
        }
        /* ========================================================
           VERIFY PRINCIPAL
        ...
        if (!principal) {
            return res.status(403).json({
                message: "Only the Principal can process student promotion",
            });
        }
        /* ========================================================
        ...
        const result = await config_1.prisma.$transaction(async (tx) => {
            ...
            ================================================== */
            if (student.class.academicYearId !==
                fromAcademicYearId) {
                throw new Error("STUDENT_NOT_IN_SOURCE_YEAR");
            }
            /* ==================================================
               PENDING FEE
            ================================================== */
            ...
            if (status !== "NOT_PROMOTED") {
                ...
                if (!targetSection) {
                    throw new Error("TARGET_SECTION_NOT_FOUND");
                }
            }
            /* ==================================================
               CHECK EXISTING DECISION
            ================================================== */
            const existingPromotion = await tx.studentPromotion.findFirst({
                where: {
                    schoolId,
                    studentId,
                    fromAcademicYearId,
            ...
            if (existingPromotion) {
                throw new Error("PROMOTION_ALREADY_PROCESSED");
            }
            /* ==================================================
               CREATE HISTORY
            ================================================== */
        ...
        ======================================================== */
        return res.status(200).json({
            message: "Student promotion processed successfully",
            promotion: result.promotion,
            student: result.student,
        });
    }
    catch (error) {
        console.error("PROCESS STUDENT PROMOTION ERROR:", error);
        /* ========================================================
           KNOWN ERRORS
        ======================================================== */
        switch (error?.message) {
            ...
            case "STUDENT_NOT_IN_SOURCE_YEAR":
                return res.status(400).json({
                    message: "Student does not belong to the selected source academic year",
                });
            case "PENDING_FEE":
                return res.status(400).json({
                    message: "Student has pending fees and cannot be promoted",
                });
            case "TARGET_CLASS_NOT_FOUND":
                return res.status(404).json({
                    message: "Target class not found for the target academic year",
                });
            case "TARGET_SECTION_NOT_FOUND":
                return res.status(404).json({
                    message: "Target section not found for the selected class",
                });
            case "PROMOTION_ALREADY_PROCESSED":
                return res.status(409).json({
                    message: "Promotion decision has already been processed for this student",
                });
            default:
                return (0, utils_1.handleErr)(error, res);
        }
...
============================================================ */
const processBulkStudentPromotion = async (req, res) => {
    try {
        const { fromAcademicYearId, toAcademicYearId, students, } = req.body;
        const schoolId = getSchoolId(req);
        const promotedByUserId = req.user?.id;
        /* ========================================================
           BASIC VALIDATION
        ...
        if (!Array.isArray(students) ||
            students.length === 0) {
            return res.status(400).json({
                message: "Student promotion data is required",
            });
        }
        /* ========================================================
           VERIFY PRINCIPAL
        ...
        if (uniqueStudentIds.size !==
            studentIds.length) {
            return res.status(400).json({
                message: "Duplicate student entries found",
            });
        }
        /* ========================================================
           VALID STATUSES
        ...
        for (const item of students) {
            ...
            if (!validStatuses.includes(item.status)) {
                return res.status(400).json({
                    message: `Invalid promotion status for student ${item.studentId}`,
                });
            }
            if (item.status !==
                "NOT_PROMOTED" &&
                (!item.toClassId ||
                    !item.toSectionId)) {
                return res.status(400).json({
                    message: `Target class and section are required for student ${item.studentId}`,
                });
            }
        }
        /* ========================================================
        ...
        const result = await config_1.prisma.$transaction(async (tx) => {
            ...
            /* ==================================================
               VERIFY ALL STUDENTS
            ================================================== */
            const dbStudentMap = new Map(dbStudents.map(student => [
                student.id,
                student,
            ]));
            for (const studentId of studentIds) {
                if (!dbStudentMap.has(studentId)) {
            ...
            for (const student of dbStudents) {
                if (student.class
                    .academicYearId !==
                    fromAcademicYearId) {
                    throw new Error(`STUDENT_NOT_IN_SOURCE_YEAR:${student.id}`);
                }
            }
            /* ==================================================
               CHECK EXISTING PROMOTIONS
            ================================================== */
            const existingPromotions = await tx.studentPromotion.findMany({
                where: {
                    schoolId,
                    studentId: {
                        in: studentIds,
            ...
            if (existingPromotions.length > 0) {
                throw new Error(`PROMOTION_ALREADY_PROCESSED:${existingPromotions
            ...
            const targetClassMap = new Map(targetClasses.map(item => [
                item.id,
                item,
            ]));
            for (const item of students) {
                ...
                if (!targetClassMap.has(item.toClassId)) {
                    throw new Error(`TARGET_CLASS_NOT_FOUND:${item.toClassId}`);
                }
            }
            /* ==================================================
            ...
            const targetSectionMap = new Map(targetSections.map(item => [
                item.id,
                item,
            ]));
            for (const item of students) {
                ...
                const section = targetSectionMap.get(item.toSectionId);
                if (!section) {
                    throw new Error(`TARGET_SECTION_NOT_FOUND:${item.toSectionId}`);
                }
                if (section.classId !==
                    item.toClassId) {
                    throw new Error(`SECTION_CLASS_MISMATCH:${item.studentId}`);
                }
            }
            /* ==================================================
               PENDING FEE CHECK
            ...
            for (const item of students) {
                ...
                const student = dbStudentMap.get(item.studentId);
                const pendingAmount = Number(student.pendingAmount ??
                    0);
            ...
            for (const item of students) {
                const student = dbStudentMap.get(item.studentId);
                const targetClass = item.status !==
                    "NOT_PROMOTED"
                    ? targetClassMap.get(item.toClassId)
                    : null;
                const targetSection = item.status !==
                    "NOT_PROMOTED"
                    ? targetSectionMap.get(item.toSectionId)
                    : null;
                /* ----------------------------------------------
                   CREATE HISTORY
                ---------------------------------------------- */
        ...
        if (message.startsWith("STUDENT_NOT_FOUND:")) {
            return res.status(404).json({
                message: `Student not found: ${message.split(":")[1]}`,
            });
        }
        if (message.startsWith("STUDENT_NOT_IN_SOURCE_YEAR:")) {
            return res.status(400).json({
                message: "One or more students do not belong to the selected source academic year",
            });
        }
        if (message.startsWith("PROMOTION_ALREADY_PROCESSED:")) {
            return res.status(409).json({
                message: "One or more students have already been processed for this academic-year transition",
            });
        }
        if (message.startsWith("TARGET_CLASS_NOT_FOUND:")) {
            return res.status(404).json({
                message: "One or more target classes were not found",
            });
        }
        if (message.startsWith("TARGET_SECTION_NOT_FOUND:")) {
            return res.status(404).json({
                message: "One or more target sections were not found",
            });
        }
        if (message.startsWith("SECTION_CLASS_MISMATCH:")) {
            return res.status(400).json({
                message: "A selected section does not belong to its selected target class",
            });
        }
        if (message.startsWith("PENDING_FEE:")) {
            return res.status(400).json({
                message: `Student ${message.split(":")[1]} has pending fees and cannot be promoted`,
            });
        }
        if (message.startsWith("INVALID_PENDING_AMOUNT:")) {
            return res.status(400).json({
                message: "Invalid pending fee amount for one of the students",
            });
        }
        return (0, utils_1.handleErr)(error, res);
    }
...
/* ============================================================
   EXPORT
============================================================ */
exports.studentPromotionControllers = {
