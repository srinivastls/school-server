...
const getPendingDues = async (req, res) => {
    try {
        const { classNumber, perc = "100", } = req.query;
        /* ========================================================
           VALIDATE PERCENTAGE
        ======================================================== */
        const percentage = Number(perc);
        if (!Number.isFinite(percentage) ||
            ...
            percentage > 100) {
            return res.status(400).json({
                message: "Invalid pending percentage",
            });
        }
        /* ========================================================
           SCHOOL
        ...
        for (const classDetails of classes) {
            ...
            for (const student of classDetails.students) {
                ...
                ==================================================== */
                const unpaidPercentage = pendingAmount *
                    (100 / totalFee);
                /* ====================================================
                   PERCENTAGE FILTER
                ==================================================== */
                ...
                result.push({
                    ...
                    sectionName: student.sectionId ??
                        "",
                    totalFee,
                    unpaidPercentage: Number(unpaidPercentage.toFixed(2)),
                    pendingAmount,
                    pendingTuitionFee: Number(student.pendingTuitionFee ??
                        0),
                    pendingTextbookFee: Number(student.pendingTextbookFee ??
                        0),
                    pendingNotebookFee: Number(student.pendingNotebookFee ??
                        0),
                    pendingDiaryAmount: Number(student.pendingDiaryAmount ??
                        0),
                    tie: {
                        amount: Number(student.tieAmount ??
                            0),
                        pendingAmount: Number(student.tiePendingAmount ??
                            0),
                    },
                    belt: {
                        amount: Number(student.beltAmount ??
                            0),
                        pendingAmount: Number(student.beltPendingAmount ??
                            0),
                    },
                    arrears: {
                        amount: Number(student.arrearsAmount ??
                            0),
                        pendingAmount: Number(student.arrearsPendingAmount ??
                            0),
                    },
                });
            }
        ...
        ======================================================== */
        result.sort((a, b) => {
            const classCompare = String(a.classNumber).localeCompare(String(b.classNumber), undefined, {
                numeric: true,
            });
            if (classCompare !== 0) {
                return classCompare;
        ...
        /* ========================================================
           SUMMARY
        ======================================================== */
        const totalPendingAmount = result.reduce((sum, student) => sum +
            Number(student.pendingAmount), 0);
        const totalPayableAmount = result.reduce((sum, student) => sum +
        ...
        result.forEach((student) => {
            const key = String(student.classNumber);
            ...
            if (existing) {
                ...
                classSummaryMap.set(key, {
                    classNumber: key,
                    studentCount: 1,
                    pendingAmount: Number(student.pendingAmount),
                });
            }
        });
