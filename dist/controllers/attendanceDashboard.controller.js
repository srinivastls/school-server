...
exports.attendanceDashboardControllers = void 0;
...
const getAttendanceDashboard = async (req, res) => {
    try {
        ...
        /* ======================================================
           STUDENT PERCENTAGES
        ====================================================== */
        const studentMarked = studentSummary.marked;
        const studentAttendancePercentage = studentMarked > 0
            ? Number(((studentSummary.present +
                studentSummary.late +
                studentSummary.halfDay) /
                studentMarked) *
                100).toFixed(2)
            : 0;
        const studentAbsentPercentage = studentMarked > 0
            ? Number((studentSummary.absent /
                studentMarked) *
                100).toFixed(2)
            : 0;
        /* ======================================================
           TEACHER PERCENTAGES
        ====================================================== */
        const teacherMarked = teacherSummary.marked;
        const teacherAttendancePercentage = teacherMarked > 0
            ? Number(((teacherSummary.present +
                teacherSummary.halfDay) /
                teacherMarked) *
                100).toFixed(2)
            : 0;
        const teacherAbsentPercentage = teacherMarked > 0
            ? Number((teacherSummary.absent /
                teacherMarked) *
                100).toFixed(2)
            : 0;
        /* ======================================================
        ...
        const classWise = Array.from(classMap.values()).map(item => {
            const marked = item.marked;
            return {
                ...item,
                attendancePercentage: marked > 0
                    ? Number(((item.present +
                        item.late +
                        item.halfDay) /
                        marked) *
        ...
        students.forEach(student => {
            ...
            if (!sectionMap.has(sectionId)) {
                sectionMap.set(sectionId, {
                    ...
                    classNumber: student.class
                        .classNumber,
                    classDisplayName: student.class
                        .displayName,
            ...
            const item = sectionMap.get(sectionId);
            item.totalStudents++;
            const attendance = student
                .attendances?.[0];
            if (!attendance) {
        ...
        const sectionWise = Array.from(sectionMap.values()).map(item => {
            const marked = item.marked;
            return {
                ...item,
                attendancePercentage: marked > 0
        ...
        return res.status(200).json({
            ...
            students: {
                ...
                absentPercentage: studentAbsentPercentage,
                classWise,
                sectionWise,
            },
            teachers: {
                ...
                absentPercentage: teacherAbsentPercentage,
            },
        });
    }
    catch (error) {
...
/* ============================================================
   EXPORT
============================================================ */
exports.attendanceDashboardControllers = {
