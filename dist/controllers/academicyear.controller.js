...
const createAcademicYear = async (req, res) => {
    try {
        ...
        /* --------------------------------------------------------
           VALIDATE NAME
        -------------------------------------------------------- */
        const academicYearName = name?.trim();
        if (!academicYearName) {
            return res.status(400).json({
        ...
        if (!parsedStartDate) {
            return res.status(400).json({
                message: "Invalid start date. Use DD/MM/YYYY",
            });
        }
        if (!parsedEndDate) {
            return res.status(400).json({
                message: "Invalid end date. Use DD/MM/YYYY",
            });
        }
        /* --------------------------------------------------------
           DATE ORDER
        ...
        /* --------------------------------------------------------
           DUPLICATE NAME
        -------------------------------------------------------- */
        const existingYear = await config_1.prisma.academicYear.findUnique({
            where: {
                schoolId_name: {
                    schoolId,
        ...
        -------------------------------------------------------- */
        const overlappingYear = await config_1.prisma.academicYear.findFirst({
            where: {
                schoolId,
                AND: [
                    {
        ...
        if (overlappingYear) {
            return res.status(409).json({
                message: `Academic year dates overlap with ${overlappingYear.name}`,
            });
        }
        /* ========================================================
           CREATE
        ======================================================== */
        const academicYear = await config_1.prisma.$transaction(async (tx) => {
            /*
...
const setCurrentAcademicYear = async (req, res) => {
    try {
        ...
        const academicYearId = String(req.params?.academicYearId ??
            "").trim();
        if (!academicYearId) {
            return res.status(400).json({
...
/* ============================================================
   POPULATE
============================================================ */
const populateAcademicYear = async (req, res) => {
    try {
        const schoolId = getSchoolId(req);
        if (!schoolId) {
            return res.status(400).json({
        ...
        if (academicYears.length !== 2) {
            return res.status(404).json({
                message: "Source or target academic year not found",
            });
        }
        const sourceYear = academicYears.find(year => year.id ===
            sourceAcademicYearId);
        const targetYear = academicYears.find(year => year.id ===
            targetAcademicYearId);
        /* ========================================================
           TRANSACTION
        ======================================================== */
        const result = await config_1.prisma.$transaction(async (tx) => {
            ...
            if (sourceClasses.length === 0) {
                throw new Error("SOURCE_CLASSES_NOT_FOUND");
            }
            /* ==================================================
               EXISTING TARGET CLASSES
            ================================================== */
            const existingTargetClasses = await tx.class.findMany({
                where: {
                    schoolId,
            ...
            ================================================== */
            const duplicateClasses = sourceClasses.filter(sourceClass => existingClassNumbers.has(sourceClass.classNumber));
            if (duplicateClasses.length > 0) {
                throw new Error(`TARGET_CLASSES_ALREADY_EXIST:${duplicateClasses
            ...
            for (const item of createdClasses) {
                const { sourceClass, newClass, } = item;
                /* ================================================
                   SECTIONS
                ================================================ */
                ...
                /* ================================================
                   SUBJECTS
                ================================================ */
                for (const sourceSubject of sourceClass.subjects) {
                    await tx.subject.create({
                        data: {
                            ...
                            isOptional: sourceSubject.isOptional,
                        },
                    });
                    subjectsCreated++;
                }
        ...
        ======================================================== */
        return res.status(201).json({
            message: "Academic year populated successfully",
            sourceAcademicYear: {
                id: sourceYear.id,
        ...
        console.error("POPULATE ACADEMIC YEAR ERROR:", error);
        /* ========================================================
           KNOWN ERRORS
        ======================================================== */
        if (error?.message ===
            ...
            return res.status(404).json({
                message: "No classes found in the source academic year",
            });
        }
        if (error?.message?.startsWith("TARGET_CLASSES_ALREADY_EXIST:")) {
            const classes = error.message
                .split(":")[1];
            return res.status(409).json({
                message: `The following classes already exist in the target academic year: ${classes}`,
            });
        }
        return (0, utils_1.handleErr)(error, res);
    }
