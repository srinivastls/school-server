    ...
    try {
        ...
        for (const section of classTeacherSections) {
            ...
            sectionMap.set(section.id, {
                ...
                classDisplayName: section.class.displayName,
                academicYearId: academicYear.id,
        ...
        for (const mapping of mappings) {
            ...
            if (mapping.subject &&
                ...
                sectionData.subjects.push({
                    ...
                    isOptional: mapping.subject.isOptional,
                });
            }
        }
        /* ========================================================
        ...
        const studentCounts = sectionIds.length > 0
            ? await config_1.prisma.studentAcademicEnrollment.groupBy({
                by: ["sectionId"],
                where: {
                    schoolId,
                    academicYearId: academicYear.id,
        ...
        for (const item of studentCounts) {
            studentCountMap.set(item.sectionId, item._count._all);
        }
        /* ========================================================
        ...
        const sections = Array.from(sectionMap.values()).map((section) => ({
            ...section,
            totalStudents: studentCountMap.get(section.sectionId) ?? 0,
        }));
        /* ========================================================
           6. SORT
        ======================================================== */
        sections.sort((a, b) => {
            const classCompare = a.classNumber.localeCompare(b.classNumber, undefined, {
                numeric: true,
            });
            if (classCompare !== 0) {
                return classCompare;
        ...
        /* ========================================================
           7. SUMMARY
        ======================================================== */
        const totalStudents = sections.reduce((total, section) => total +
            section.totalStudents, 0);
        const totalSubjects = new Set(mappings.map((mapping) => mapping.subject.id)).size;
        const classTeacherSectionCount = sections.filter((section) => section.isClassTeacher).length;
        /* ========================================================
           RESPONSE
        ======================================================== */
        return res.status(200).json({
            ...
            summary: {
                ...
                totalStudents,
                totalSubjects,
                classTeacherSections: classTeacherSectionCount,
            },
            sections,
        });
    }
...
const getTeacherSectionStudents = async (req, res) => {
    try {
        ...
        -------------------------------------------------------- */
        const isClassTeacher = section.classTeacherId ===
            teacherUserId;
        ...
        const students = enrollments.map((enrollment) => ({
            ...
            emergencyContact: enrollment.student
                .emergencyContact,
        ...
        console.error("GET TEACHER SECTION STUDENTS ERROR:", error);
        return (0, utils_1.handleErr)(error, res);
    }
};
/* ============================================================
...
============================================================ */
const teacherHasSectionAccess = async (teacherUserId, schoolId, sectionId, academicYearId) => {
    /* ----------------------------------------------------------
       CLASS TEACHER
    ---------------------------------------------------------- */
    const section = await config_1.prisma.section.findFirst({
 ...
 */
const getTeacherSectionAttendance = async (req, res) => {
    try {
        /* --------------------------------------------------------
           AUTHENTICATED TEACHER
        -------------------------------------------------------- */
        ...
        const attendanceDate = parseAttendanceDate(dateInput);
        if (!attendanceDate) {
            return res.status(400).json({
                message: "Valid date is required. Use YYYY-MM-DD or DD/MM/YYYY",
            });
        }
        /* --------------------------------------------------------
           CURRENT ACADEMIC YEAR
        ...
        if (!section) {
            return res.status(404).json({
                message: "Section not found for current academic year",
            });
        }
        /* --------------------------------------------------------
        ...
        if (section.class.isCompleted) {
            return res.status(400).json({
                message: "Attendance cannot be marked for a completed class",
            });
        }
        /* --------------------------------------------------------
           TEACHER PERMISSION
        -------------------------------------------------------- */
        const hasAccess = await teacherHasSectionAccess(teacherUserId, schoolId, sectionId, academicYear.id);
        if (!hasAccess) {
            return res.status(403).json({
                message: "You are not assigned to this section",
            });
        ...
        const attendance = studentIds.length > 0
            ? await config_1.prisma.attendance.findMany({
                where: {
                    schoolId,
                    academicYearId: academicYear.id,
                    sectionId,
        ...
        const summary = {
            ...
            marked: students.filter((student) => student.attendance !== null).length,
            unmarked: students.filter((student) => student.attendance === null).length,
            present: students.filter((student) => student.attendance?.status ===
                "PRESENT").length,
            absent: students.filter((student) => student.attendance?.status ===
                "ABSENT").length,
            late: students.filter((student) => student.attendance?.status ===
                "LATE").length,
            halfDay: students.filter((student) => student.attendance?.status ===
                "HALF_DAY").length,
            holiday: students.filter((student) => student.attendance?.status ===
                "HOLIDAY").length,
        };
        /* --------------------------------------------------------
           RESPONSE
        ...
        return res.status(200).json({
            ...
            section: {
                ...
                classNumber: section.class.classNumber,
                classDisplayName: section.class.displayName,
                isClassTeacher: section.classTeacherId ===
                    teacherUserId,
            },
            summary,
            totalStudents: students.length,
        ...
        console.error("GET TEACHER SECTION ATTENDANCE ERROR:", error);
        return (0, utils_1.handleErr)(error, res);
    }
};
/* ============================================================
 ...
 */
const saveTeacherSectionAttendance = async (req, res) => {
    try {
        /* --------------------------------------------------------
           AUTHENTICATED TEACHER
        -------------------------------------------------------- */
        ...
        const validStudentIds = new Set(enrollments.map((item) => item.studentId));
        /* --------------------------------------------------------
           VALIDATE PAYLOAD
        -------------------------------------------------------- */
        ...
        for (const item of attendance) {
            if (!item || typeof item !== "object") {
                return res.status(400).json({
                    message: "Invalid attendance item",
                });
            }
            ...
            if (!studentId) {
                return res.status(400).json({
                    message: "studentId is required for every attendance record",
                });
            }
            ...
            if (seenStudentIds.has(studentId)) {
                return res.status(400).json({
                    message: `Duplicate attendance record for student ${studentId}`,
                });
            }
            ...
            if (item.remark !== undefined &&
                ...
                return res.status(400).json({
                    message: `Invalid remark for student ${studentId}`,
                });
            }
        }
        /* --------------------------------------------------------
        ...
        console.error("SAVE TEACHER SECTION ATTENDANCE ERROR:", error);
        return (0, utils_1.handleErr)(error, res);
    }
};
const getTeacherStudentAttendanceHistory = async (req, res) => {
    try {
        const teacherUserId = req.user?.id;
        const schoolId = req.user?.schoolId;
        ...
        const enrollment = await config_1.prisma.studentAcademicEnrollment.findFirst({
            where: {
                schoolId,
                academicYearId: academicYear.id,
                studentId,
        ...
        const workingDays = summary.present +
            summary.absent +
            summary.late +
            summary.halfDay;
 ...
 * to the authenticated teacher for the current academic year.
 */
const getTeacherMyStudents = async (req, res) => {
    try {
        const teacherUserId = req.user?.id;
        const schoolId = req.user?.schoolId;
        if (!teacherUserId || !schoolId) {
        ...
        const sectionSummary = sections.map((section) => ({
            ...
            classNumber: section.class.classNumber,
            classDisplayName: section.class.displayName,
            totalStudents: students.filter((student) => student.sectionId === section.id).length,
        }));
        const uniqueClassIds = new Set(sections.map((section) => section.class.id));
        return res.status(200).json({
            academicYear,
            summary: {
                totalStudents: students.length,
        ...
        console.error("GET TEACHER MY STUDENTS ERROR:", error);
        return (0, utils_1.handleErr)(error, res);
    }
};
/**
...
const getMyTeacherAttendance = async (req, res) => {
    try {
        ...
        if (month) {
            ...
            where.date = {
                gte: monthStart > academicYear.startDate
                    ? monthStart
                    : academicYear.startDate,
                lt: nextMonthStart < academicYear.endDate
                    ? nextMonthStart
 ...
 */
const getMyTeacherAttendanceToday = async (req, res) => {
    try {
        const teacherUserId = req.user?.id;
        const schoolId = req.user?.schoolId;
        if (!teacherUserId) {
        ...
        const attendance = await config_1.prisma.teacherAttendance.findUnique({
            where: {
                schoolId_teacherUserId_date: {
                    schoolId,
                    teacherUserId,
