...
const markStudentAttendance = async (req, res) => {
    try {
        ...
        if (uniqueStudentIds.size !==
            studentIds.length) {
            return res.status(400).json({
                message: "Duplicate student attendance entries found",
            });
        }
        /* ======================================================
           VERIFY MARKING USER
        ...
        if (students.length !==
            ...
            const invalidStudent = studentIds.find(id => !foundIds.has(id));
            return res.status(400).json({
                message: `Student ${invalidStudent} does not belong to the selected class/section`,
            });
        }
        /* ======================================================
           SAVE
        ...
        const summary = {
            total: saved.length,
            present: saved.filter(item => item.status ===
                "PRESENT").length,
            absent: saved.filter(item => item.status ===
                "ABSENT").length,
            late: saved.filter(item => item.status ===
                "LATE").length,
            halfDay: saved.filter(item => item.status ===
                "HALF_DAY").length,
            holiday: saved.filter(item => item.status ===
                "HOLIDAY").length,
        };
...
const getStudentsForAttendance = async (req, res) => {
    try {
        ...
        /* ========================================================
           QUERY
        ======================================================== */
        const { classId, sectionId, academicYearId, date, } = req.query;
        if (!classId) {
            return res.status(400).json({
        ...
        /* ========================================================
           DATE
        ======================================================== */
        const attendanceDate = (0, dayjs_1.default)(String(date), "DD/MM/YYYY", true);
        if (!attendanceDate.isValid()) {
            return res.status(400).json({
                message: "Invalid date. Use DD/MM/YYYY",
            });
        }
        const dateValue = attendanceDate
            .startOf("day")
            .toDate();
        /* ========================================================
        ...
        if (classDetails.isCompleted) {
            return res.status(400).json({
                message: "This class is completed",
            });
        }
        /* ========================================================
           VERIFY SECTION
        ...
        if (!section) {
            return res.status(404).json({
                message: "Section not found for selected class",
            });
        }
        /* ========================================================
        ...
        ======================================================== */
        const teacherMapping = await config_1.prisma.teacherSubjectMapping.findFirst({
            where: {
                schoolId,
            ...
            return {
                ...
                attendance: existingAttendance
                    ? {
                        ...
                        remark: existingAttendance.remark,
                        markedByUserId: existingAttendance
                            .markedByUserId,
                        createdAt: existingAttendance
                            .createdAt,
                        updatedAt: existingAttendance
                            .updatedAt,
                    }
                    : null,
            };
        ...
        const summary = {
            total: result.length,
            present: result.filter(student => student.attendance
                ?.status ===
                "PRESENT").length,
            absent: result.filter(student => student.attendance
                ?.status ===
                "ABSENT").length,
            late: result.filter(student => student.attendance
                ?.status ===
                "LATE").length,
            halfDay: result.filter(student => student.attendance
                ?.status ===
                "HALF_DAY").length,
            holiday: result.filter(student => student.attendance
                ?.status ===
                "HOLIDAY").length,
            notMarked: result.filter(student => !student.attendance).length,
        };
        /* ========================================================
           RESPONSE
        ======================================================== */
        return res.status(200).json({
            date: attendanceDate.format("DD/MM/YYYY"),
            academicYear: {
                id: academicYear.id,
