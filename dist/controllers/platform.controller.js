...
const updateSchoolStatus = async (req, res) => {
    try {
        ...
        if (!schoolId || !status) {
            return res.status(400).json({
                message: "schoolId and status are required",
            });
        }
        if (status !== "ACTIVE" &&
            status !== "SUSPENDED") {
        ...
        if (school.status === status) {
            return res.status(400).json({
                message: `School is already ${status}`,
            });
        }
        const updatedSchool = await config_1.prisma.$transaction(async (tx) => {
...
const createSchool = async (req, res) => {
    try {
        const { code, name, address, contactEmail, contactPhone, logoUrl, board, subscriptionPlan, maxStudents, maxStaffAccounts, gracePeriodDays, } = req.body;
        ...
        if (!code || !name) {
            return res.status(400).json({
                message: "School code and school name are required",
            });
        }
        const normalizedCode = code.trim().toUpperCase();
        const normalizedEmail = contactEmail?.trim().toLowerCase() || null;
        const existingSchool = await config_1.prisma.school.findUnique({
            where: {
                code: normalizedCode,
            },
        });
        ...
        const school = await config_1.prisma.$transaction(async (tx) => {
    ...
    try {
        ...
        if (school.status === "SUSPENDED" ||
            ...
            return res.status(403).json({
                message: "Cannot create principal for a suspended or expired school",
            });
        }
        ...
        const principal = await config_1.prisma.$transaction(async (tx) => {
